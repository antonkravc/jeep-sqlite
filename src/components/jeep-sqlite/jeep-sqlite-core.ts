...
class EventEmitter<T> {
  ...
  constructor(readonly __name: string){
  }

  emit(data?: T): CustomEvent<T> {
    const event = new CustomEvent<T>(this.__name, data);
    this.emitter.dispatchEvent(event);
    return event;
  }

  addEventListener(callback: (event: CustomEvent<T>) => void) {
    this.emitter.addEventListener(this.__name, callback);
  }
}
...
export class JeepSqlite {
  ...
  //     bubbles: true,
  // })
  importProgress = new EventEmitter<JsonProgressListener>("jeepSqliteImportProgress");
  // @Event({eventName:'jeepSqliteExportProgress',
  //     composed: true,
  //     cancelable: true,
  //     bubbles: true,
  // })
  exportProgress = new EventEmitter<JsonProgressListener>("jeepSqliteExportProgress");
  // @Event({eventName:'jeepSqliteHTTPRequestEnded',
  //     composed: true,
  //     cancelable: true,
  //     bubbles: true,
  // })
  HTTPRequestEnded = new EventEmitter<HTTPRequestEndedListener>("jeepSqliteHTTPRequestEnded");
  // @Event({eventName:'jeepSqlitePickDatabaseEnded',
  //     composed: true,
  //     cancelable: true,
  //     bubbles: true,
  // })
  PickDatabaseEnded = new EventEmitter<PickOrSaveDatabaseEndedListener>("jeepSqlitePickDatabaseEnded");
  // @Event({eventName:'jeepSqliteSaveDatabaseToDisk',
  //     composed: true,
  //     cancelable: true,
  //     bubbles: true,
  // })
  SaveDatabaseEnded = new EventEmitter<PickOrSaveDatabaseEndedListener>("jeepSqliteSaveDatabaseToDisk");

  private EVENTS = {
    jeepSqliteImportProgress: this.importProgress,
    jeepSqliteExportProgress: this.exportProgress,
    jeepSqliteHTTPRequestEnded: this.HTTPRequestEnded,
    jeepSqlitePickDatabaseEnded: this.PickDatabaseEnded,
    jeepSqliteSaveDatabaseToDisk: this.SaveDatabaseEnded
  };

  addEventListener(name: string, callback: (event: CustomEvent) => void) {
    const event = this.EVENTS[name as keyof typeof this.EVENTS];
    if (!event) {
      console.warn("No event emitter for", name);
      return;
    }
